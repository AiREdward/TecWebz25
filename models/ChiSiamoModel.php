<?php
class ChiSiamoModel {
    public function getData() {
        return [
            'title' => 'Chi Siamo',
            'header' => 'Chi Siamo',
            'content' => '<h2>Benvenuti nel nostro mondo gaming!</h2>
                <p>Siamo uno shop online specializzato nel settore gaming, con la passione per i videogiochi e tutto ciò che riguarda il mondo del gaming digitale.</p>
                
                <h3>I Nostri Servizi</h3>
                
                <h4>Shop Online</h4>
                <p>Offriamo una vasta gamma di prodotti per il gaming: dalle ultime console ai videogiochi più recenti, accessori di alta qualità, componenti per PC gaming e merchandise ufficiale. Tutti i nostri prodotti sono selezionati con cura per garantire la migliore esperienza di gioco possibile.</p>
                
                <h4>Servizio di Noleggio</h4>
                <p>Non sei sicuro di voler acquistare un gioco o una console? Il nostro servizio di noleggio ti permette di provare i prodotti prima dell\'acquisto. Offriamo noleggi a breve e lungo termine per console, videogiochi e accessori, con tariffe competitive e condizioni flessibili.</p>
                
                <h4>Valutazione dell\'Usato</h4>
                <p>Hai dispositivi o giochi che non usi più? Il nostro servizio di valutazione dell\'usato ti offre la possibilità di vendere i tuoi prodotti gaming a prezzi equi. Valutiamo con attenzione ogni articolo, garantendo il miglior prezzo possibile in base alle condizioni e alla domanda di mercato.</p>
                
                <h3>La Nostra Missione</h3>
                <p>La nostra missione è rendere il gaming accessibile a tutti, offrendo prodotti di qualità, servizi personalizzati e un\'assistenza clienti impeccabile. Ci impegniamo a creare una comunità di appassionati, dove condividere esperienze e scoprire nuove avventure nel mondo dei videogiochi.</p>
                
                <h3>Perché Sceglierci</h3>
                <ul>
                    <li>Esperti del settore con anni di esperienza</li>
                    <li>Prodotti selezionati e di qualità garantita</li>
                    <li>Prezzi competitivi e offerte esclusive</li>
                    <li>Servizio clienti dedicato e professionale</li>
                    <li>Spedizioni rapide e sicure</li>
                </ul>
                
                <p>Vieni a trovarci online o contattaci per qualsiasi informazione. Siamo qui per rendere la tua esperienza di gaming indimenticabile!</p>'
        ];
    }
}
?>